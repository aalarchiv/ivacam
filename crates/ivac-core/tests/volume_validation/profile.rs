...
fn profile_outside_op() -> ivac_core::project::Op {
    op_single_pass(
        1,
        "Profile-Outside",
        OpKind::Profile {
...
fn profile_outside_circle_volume_matches_closed_form() {
    ...
    let n_bytes = dump_stl(
        &hm,
        "/tmp/ivac_profile_outside_circle.stl",
        -(STOCK_THICK as f32),
    );
    eprintln!("[me1m/profile] STL → /tmp/ivac_profile_outside_circle.stl ({n_bytes} bytes)");
