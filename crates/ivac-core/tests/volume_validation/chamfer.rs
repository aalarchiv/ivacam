...
use ivac_core::cam::chamfer::{chamfer_depth_capped, chamfer_width_cap_mm};
use ivac_core::project::OpKind;
...
fn chamfer_op() -> ivac_core::project::Op {
...
fn chamfer_closed_rectangle_volume_matches_closed_form() {
    ...
    let n_bytes = dump_stl(&hm, "/tmp/ivac_chamfer_closed.stl", -(STOCK_THICK as f32));
    eprintln!("[7krz/closed] STL → /tmp/ivac_chamfer_closed.stl ({n_bytes} bytes)");
...
fn chamfer_open_edge_emits_lateral_cut_at_each_pass() {
    ...
    let cut_zs: Vec<f64> = resp
        ...
        .filter(|s| {
            matches!(
                s.kind,
                ivac_core::gcode::preview::MoveKind::Cut | ivac_core::gcode::preview::MoveKind::Arc
            )
        })
        .map(|s| s.to.z)
        .collect();
