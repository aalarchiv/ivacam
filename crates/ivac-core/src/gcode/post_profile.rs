...
/// `PostProcessorKind` is unaffected — ivac's linuxcnc / grbl / hpgl
...
impl PostProfile {
    ...
    pub fn grbl_default() -> Self {
        Self {
            ...
            file_extension: Some("nc".into()),
            line_ending: Some("\n".into()),
            program_start: Some("; ivac <version> — GRBL".into()),
...
impl TokenCtx {
    /// Default that reads the ivac version from the `CARGO_PKG` env.
    /// Kept distinct from `Default` so the env lookup only happens
    /// on the explicit-call path.
    #[must_use]
    pub fn with_ivac_version() -> Self {
        Self {
...
mod tests {
    ...
    fn substitute_replaces_basic_tokens() {
        ...
        let s = substitute("T<t> (<n>) F<f> S<s> ivac <version>", &ctx);
        assert_eq!(s, "T7 (3mm endmill) F800 S18000 ivac 0.1.0");
    }
