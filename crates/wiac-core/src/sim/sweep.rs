...
/// for tests and progress reporting). Rapid moves do not carve; they
/// run through `rapid_check` and emit a `RapidThroughMaterial` warning
/// when the cutter would pass through material at rapid speed.
...
pub fn sweep_range(
    ...
    for (offset, seg) in segments[lo..hi].iter().enumerate() {
        total += sweep_segment(heightmap, seg, profile, lo + offset, diagnostics);
    }
    total
}
