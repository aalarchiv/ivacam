//! `wiac` — headless converter and JSON-API surface for the Rust core.
//!
//! Subcommands:
//!   * `wiac import <file>`            — emit /import-shaped JSON to stdout
//!   * `wiac generate <file> [--post]` — emit /generate-shaped JSON
...
fn print_help() {
    eprintln!(
        "wiac — wiaConstructor headless CLI\n\n\
         usage:\n  \
...
fn build_offsets(
    ...
    for (idx, obj) in objects.iter().enumerate() {
        ...
        if pocket {
            for mut o in pocket_for_object(obj, radius, false, 6) {
