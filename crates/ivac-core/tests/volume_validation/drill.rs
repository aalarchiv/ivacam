...
use ivac_core::project::{DrillCycle, OpKind};
...
fn drill_op() -> ivac_core::project::Op {
    op_single_pass(
        1,
        "Drill",
        OpKind::Drill {
...
fn drill_three_holes_volume_matches_closed_form() {
    ...
    let n_bytes = dump_stl(&hm, "/tmp/ivac_drill_three.stl", -(STOCK_THICK as f32));
    eprintln!("[me1m/drill] STL → /tmp/ivac_drill_three.stl ({n_bytes} bytes)");
